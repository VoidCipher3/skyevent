...
const UI = {
  tw: {
    ...
    fetchError: (n) => `${n} 讀取失敗`,
    videoLangLabels: { en: "English", zh: "繁體中文", ja: "日本語" },
    ...
    refreshError: "觸發失敗，請稍後再試。", refreshNotConfigured: "尚未設定刷新服務。",
  },
  cn: {
    ...
    fetchError: (n) => `${n} 读取失败`,
    videoLangLabels: { en: "English", zh: "简体中文", ja: "日本語" },
    ...
    refreshError: "触发失败，请稍后再试。", refreshNotConfigured: "尚未设定刷新服务。",
  },
  en: {
    ...
    videoLangLabels: { en: "English", zh: "Chinese", ja: "Japanese" },
    ...
    refreshError: "Failed to trigger, please try again later.", refreshNotConfigured: "Refresh service not configured yet.",
  },
};
...
const QUICK_FILTERS = {
  schedule: {
    tw: [["蠟燭", "candle"], ["任務", "quest"], ["收藏品", "collectible"], ["拜訪", "visit"], ["社交", "social"], ["季節", "season"]],
    cn: [["蜡烛", "candle"], ["任务", "quest"], ["收藏品", "collectible"], ["拜访", "visit"], ["社交", "social"], ["季节", "season"]],
    en: [["Candles", "candle"], ["Quests", "quest"], ["Collectibles", "collectible"], ["Visits", "visit"], ["Social", "social"], ["Seasons", "season"]],
  },
  video: {
  ...
  if (!rows.length) {
    ...
    rows.forEach(r => {
      ...
      tr.innerHTML = buildCells(T.scheduleHead,
        [tag, esc(nameFor(r)), esc(r.start), esc(r.end), esc(r.duration_label), ""],
...
// 實際載入影片本體 + 依語言載入字幕（透過第三方 CORS 代理，見下方風險說明）
function playVideoWithLang(row, langCode) {
  ...
  // 字幕網址規則是猜測性質的替換（官方影片檔名慣例），不保證每部影片都真的存在該語言字幕檔
  let finalSubUrl = row.subtitle_url || row.subtitle_base;
  if (finalSubUrl && row.has_translations && finalSubUrl.includes("_EN_") && upperLang !== "EN") {
    finalSubUrl = finalSubUrl.replace(/_EN_/g, `_${upperLang}_`);
  }

  const requestId = ++subtitleRequestId;

  if (finalSubUrl) {
    // 注意：corsproxy.io 是不受我們控制的公開第三方代理，可能隨時限流或掛掉，
    // 這裡只做「盡量顯示」，失敗時會退回提示使用者改用另開分頁的字幕按鈕。
    const proxyUrl = `https://corsproxy.io/?${encodeURIComponent(finalSubUrl)}`;
    fetch(proxyUrl)
